...
function steptwo() {
    ...
    useEffect(() => {
        ...
        fetch('/api/get_user_experiance').then((res)=>res.json()) .then((response2) => {
            let Available_Id =
              newObjectuser.unique_id === undefined
                ? UserDetails.unique_id
                : newObjectuser.unique_id
              console.log(response2.filter((x) => x?.Public_id === Available_Id))
            setExperiance(response2.filter((x) => x?.Public_id === Available_Id))
        })
...
const addExperiance = (data)=>{
    const UserDetails = JSON.parse(localStorage?.getItem('user_id'))
    let newObjectuser = JSON.parse(localStorage.getItem('user_id'))

    fetch('/api/experiance_upload', {
        headers: {
        ...
        method: 'POST',
        body: JSON.stringify({
            Public_id: UserDetails.unique_id,
        ...
        .then((response) => response.json('uploaded')).then((_)=>{
            fetch('/api/get_user_experiance').then((res)=>res.json()) .then((response2) => {
                let Available_Id =
                  newObjectuser.unique_id === undefined
                    ? UserDetails.unique_id
                        ...
                        <div className={SteptwoStyle.menu_content}>
                            ...
                            <div className={SteptwoStyle.product_form_content}>
                                ...
                                <form
                                    ...
                                    onSubmit={handleSubmit(addExperiance)}
                                >
                                    {active === 'Experience' ? (
                                         <div className="expStyle" style={{ 
                                            display: 'flex',
                                        ...
                                        <div className="MainContent_Container">
                                           ...
                                           <div className="tell_us_title">
                                            ...
                                            <div className={SteptwoStyle.product_form}>
                                                ...
                                                <button
                                                    onClick={() => {
                                                        return handleSubmit(addExperiance)
                                                    }}
                                        ...
                                        <div className='exp_view'>
                                         ...
                                         experiance.map(i => <div className='exp_countainer' style={{ 
                                            display:'list-item',
                                            background:'#fef6f2',
                                            borderRadius:'20px',
                                            height:'100px',
